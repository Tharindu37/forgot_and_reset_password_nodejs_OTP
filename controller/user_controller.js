  ...
  try {
    ...
    connection.query(
      ...
      async (err, data, fields) => {
        ...
        connection.query(
          "INSERT INTO user (id, email, password, otp, otpExpire) VALUES(null,?)",
          [[req.body.email, hashedPassword, null, null]],
          (err, data, fields) => {
            if (err) return next(new AppError(err, 500));
            res.json({
  ...
  try {
    ...
    connection.query(
      ...
      async (err, data1, fields) => {
        ...
        connection.query(
          "UPDATE user SET otp =?, otpExpire = ? WHERE email=?",
          [otp, otpExpier, req.body.email],
          (err, data2, fields) => {
            if (err) return next(new AppError(err, 500));
...
exports.user_resetPassword = (req, res, next) => {
  ...
  try {
    ...
    if (password.localeCompare(confirmPassword) != 0)
      return next(new AppError("password are not equal", 400));

    connection.query(
      "SELECT * FROM user WHERE otp=? AND otpExpire > NOW()",
      [[body.otp]],
      async (err, data, fields) => {
        if (err) return next(new AppError(err, 500));
        if (data.length == 0)
        ...
        connection.query(
          "UPDATE user SET password=?, otp=null, otpExpire=null WHERE otp=?",
          [hashedPassword, body.otp],
          async (err, data, fields) => {
            if (err) return next(new AppError(err, 500));
